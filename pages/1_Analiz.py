...
def main():
    ...
    with st.container():
        ...
        with col1:
            ...
            your_role = st.selectbox("", roles, key="your_role", label_visibility="collapsed")
        with col2:
            st.markdown("### 🧠 Zeka Modu")
            use_ai = st.toggle("AI Açıklamalar", value=st.session_state.use_ai, help="Yapay zeka ile neden analizi")
    ...
    for idx, role in enumerate(roles):
        with enemy_cols[idx]:
            st.markdown(f"<p style='color:#C89B3C; text-align:center;'>{role}</p>", unsafe_allow_html=True)
            ...
            choice = st.selectbox(f" ", available,
                                  index=available.index(current) if current in available else 0,
                                  key=f"enemy_{role}", label_visibility="collapsed")
    ...
    for idx, role in enumerate(ally_roles_to_show):
        with ally_cols[idx]:
            ...
            choice = st.selectbox(f"  ", available,
                                  index=available.index(current) if current in available else 0,
                                  key=f"ally_{role}", label_visibility="collapsed")
    ...
    # ========== HAVUZ & ANALİZ ==========
    col_pool, col_ml, col_btn = st.columns([2,1,1])
    with col_pool:
    ...
    if st.button("🚀 ANALİZ ET", type="primary", use_container_width=True):
        ...
        with st.spinner("🔮 Yapay zeka analiz yapıyor..."):
            ...
            top3 = TeamAnalyzer.get_top_picks(candidates, weaknesses, ally_data,
                                              use_ml=use_ml, enemy_champs=enemy_data)
            ...
            with card_cols[i]:
                ...
                if use_ai:
                    with st.spinner("🧠 AI analiz ediyor..."):
                        reason = generate_pick_reason(champ['name'], enemy_data, ally_data, weaknesses)
